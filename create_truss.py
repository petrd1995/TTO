...
def node_coords(node_num, all_nodes, **kwargs):
    ...
    if 'show' in kwargs:
        print(f'the coordinates for node {node_num} are: {all_nodes[node_num]}')
...
def matK(E,num_bars,bars,rB, cB,vec,Avec):
    ...
    for i in range(num_bars):
        ...
        K[slc1:slc1end, slc1:slc1end] += np.outer(vec[i], vec[i]) * E * Avec[i] / len[i]

        K[slc2:slc2end, slc2:slc2end] += np.outer(vec[i], vec[i]) * E * Avec[i] / len[i]

        K[slc1:slc1end, slc2:slc2end] = - np.outer(vec[i], vec[i]) * E * Avec[i] / len[i]

        K[slc2:slc2end, slc1:slc1end] = - np.outer(vec[i], vec[i]) * E * Avec[i] / len[i]
